...
- **编辑器「物品页」显示名汉化 827 条（本版新增）**：物品页列表的数据源是 `GL\hlm2_objects.bin` 里的 **obj 名字池**，**显示名 = 去掉开头的 `obj`**；编辑器再拿这个名字去查贴图 `spr<后缀>` / `sprElis<后缀>` / `sprELis<后缀>`，**查不到就把整条静默丢掉**（列表看起来就"变少"了）。所以正确做法是 **obj 名与贴图名「同后缀、同步、等长」改名**：只替换公共后缀并用空格补齐到原字节长度，拼名查表依旧逐字命中，**配对一条都不丢**。落地：显示名 **827 条** + 贴图资源名 **1014 条**，合计替换 **9,962 处**，全部**等长改名**（WAD 体积一字节未变），旧名残留 0、冲突 0、缺字形 0。另有 43 条人名/乐队名与 3 字节短名保留英文；
...
<sub>物品页样张为真实渲染：左侧是游戏内实际显示名，右侧是它对应的原英文名（用于辨识）。名字开头的 `!` 是排序前缀，原因见下方 FAQ。</sub>
...
**Q：物品页为什么要"改资源名"这么暴力？**
因为物品页显示的不是翻译文本，而是**资源名本身**：列表来自 `GL\hlm2_objects.bin` 里的 obj 名字池（显示名 = 去掉开头的 `obj`），编辑器再拿它去查对应贴图。也就是说**没有单独的"显示名表"可以介入**，想让列表出现中文，只能把资源名本身改成中文。关键是**不能只改贴图、不改 obj**——那样配对会断，条目会被编辑器静默丢弃（列表变短）。为了不破坏文件结构，改名严格**等长**（中文字节数不足处用空格补齐），因此 WAD 目录树、贴图池偏移、关卡文件长度前缀全部无需变动。

**Q：为什么物品名开头会有个 `!`？**
列表按 ASCII 排序，且引擎**只索引 A–Z**——中文（UTF-8 以 `0xE5` 起）会排到全部英文之后**且进不了字母索引**，看起来就像"没汉化"。加 `!`(0x21) 才能让中文条目**排到列表最前面**。这是引擎限制下的功能性处理，不是笔误。
...
**Q：为什么有些物品名很短，有些还是英文？**
旧英文名的长度就是可写预算（`Desk` 只有 4 字节、`VIPArmchair` 只有 11 字节）。要在等长前提下写中文，短名只能放 1～2 个汉字；**连 1 个汉字都放不下的 3 字节短名**（`Box`/`Cat`/`Log`/`ATM`/`Fan`/`Tub`）就保留英文原名，不生成残缺名。另有落在 exe 拼接片段上的资源名（`Biker*`/`Cobra*`/`Colombian*` 等）与不在物品译表内的武器名（`objKnife`/`objM16`/`objUzi`…）也保留英文，原因见「已知限制」。这是数学与引擎限制，而非翻译偷懒。
...
2. **部分编辑器资源名保持英文**：`objKnife`、`objKalashnikov`、`objM16`、`objUzi` 等**武器名**不在物品译表内；`Biker*`／`Cobra*`／`Colombian*` 等约 **255 条**落在 exe 拼接片段上，exe 会用片段拼出名字再查表，改了会让角色动画/贴图失效。同理，`PUBLISH_CONFIRM` 等全大写标识符与 `Tab`/`Esc`/`Del` 等按键名也未翻译。
3. **短物品名用了简写**（如 `VIPArmchair → !贵宾扶`），3 字节短名保留英文，原因见 FAQ 的"预算"一节。
4. **正文字号偏小**：原版正文字体只有 8–12 像素高，中文已放大到行高允许的上限并换用粗体，但清晰度仍不及约 20 像素的菜单文字。
5. **自制/创意工坊关卡会失效**，见上方 FAQ。
6. 离线验证做到极限：安装后逐一反查 **550 条** exe 文本、**1600 个字形**在 76 套字体中比对 0 缺字、**827 条物品显示名**全部命中、**827 条中文条目全部能查到配对贴图（断 0 条）**且旧名残留 0、未改动的文件字节级一致，所有改动字节 100% 落在各自槽位内。正式版已由作者**在真机实机验证**。
...
- **物品名改名**：以词素词典（1160 条）做最长优先匹配 + 词边界校验，生成「中文 + 英文缩写」；**不移动任何偏移**，只在原位置把 obj 池与贴图池里同一后缀的所有前缀变体（`obj` / `spr` / `sprElis` / `sprELis` / `msk`…）同步换成同字节长度的新后缀（不足处空格补齐），并用词边界校验（前后都要求非字母数字）避免误伤 exe 拼接出来的动画名。因长度完全相等，容器内 4522 个条目的偏移与关卡文件长度前缀全部无需重建。
...
Coverage: 2,889 in-game strings; 550 hardcoded exe strings (65 achievement names/descriptions + 485 level-editor labels, dialogs and warnings) patched in place with length-safe byte replacement (offsets unchanged, Steam achievement IDs preserved); 1,600 CJK glyphs injected into all 76 bitmap fonts (Microsoft YaHei Bold); and **827 level-editor item-page display names rewritten in Chinese** (plus 1,014 sprite names renamed in lockstep) — the item list is driven by the `obj` name pool in `hlm2_objects.bin`, and rows that stop resolving are silently dropped, so `obj` and sprite names had to be renamed together with **byte-length-preserving** substitutions (9,962 references updated, WAD size byte-identical).
